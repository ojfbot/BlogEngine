...
Last updated: 2026-02-27 (PR #17 second review pass)
...
| TD-009 | HIGH | security | `packages/api/src/routes/v2/auth.ts` | `POST /api/v2/auth/token` issues a JWT for any `userId` without credential verification. **Blocks Phase C merge.** Must not be reachable from an external network until credential verification is added. A firewall or network policy MUST gate this endpoint before any staging/production deployment. | M | open |
...
| TD-011 | MEDIUM | security | `packages/api/src/routes/v2/threads.ts` | Thread routes do not filter by `req.userId` — any authenticated user can read or delete any thread. **Blocks Phase C merge.** Add `userId` filter to all GET/DELETE thread queries before Phase C is released to users. | S | open |
...
| TD-017 | MEDIUM | security | `packages/api/src/routes/v2/posts.ts` | `GET /api/posts` calls `threadService.listThreads()` without a `userId` filter — returns thread titles from all users. **Blocks Phase C merge.** Add `req.userId` filter or replace with a real posts model in Phase D. | S | open |
...
- **TD-009, TD-011, TD-017** explicitly block Phase C merge (auth bypass, cross-user data exposure). These must be closed before Phase C is deployed to any external network.
